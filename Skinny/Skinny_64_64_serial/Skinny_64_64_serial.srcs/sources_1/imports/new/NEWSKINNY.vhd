...
ARCHITECTURE Behavioral OF Skinny_64_64 IS
...
PORT (
    ...
    -- not all elements are to be xored 
    -- the same counter used for the loaing phase is used to spare logic
    AddRoundTweakey_TWEAKEY_IN : IN std_logic_vector(3 DOWNTO 0);  -- input for the key word to be xored 
    AddRoundTweakey_IS_IN : IN std_logic_vector(3 DOWNTO 0);   -- input for the plaintext word 
...
SIGNAL TW_REG_parallel_in_buf : std_logic_vector(31 DOWNTO 0);
SIGNAL TW_REG_parallel_out_buf : std_logic_vector(31 DOWNTO 0);
SIGNAL TW_REG_enable_parallel_loading_buf : std_logic;
SIGNAL TW_REG_serial_in_buf : std_logic_vector(3 DOWNTO 0);
SIGNAL TW_REG_serial_out_buf : std_logic_vector(3 DOWNTO 0);
...
BEGIN
    ...
    WHEN loading =>     
        ...
        -- State transition
        IF (cnt_out_buf = b"01111") THEN --count up tp 16     
            nx_state <= idle;        
        ELSE        
            nx_state <= loading;        
        END IF;        
    ...
    WHEN idle =>  
        -- IS REG   
        IF (lfsr_out_buf = b"111000") THEN  -- the cipher has done encrypting !!
            IS_enable1_buf <= '1';
            IS_enable2_buf <= '1';
            IS_enable3_buf <= '1';
            IS_enable4_buf <= '1';
        ...
        ---- stop shifting the TW reg in idle       
    ...
    WHEN processing => 
        -- perform SubCells, AddRoundTweakey and AddConstants sequentially
