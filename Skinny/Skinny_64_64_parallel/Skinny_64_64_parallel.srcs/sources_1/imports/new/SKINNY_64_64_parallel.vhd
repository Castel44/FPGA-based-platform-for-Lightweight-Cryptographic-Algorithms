...
ENTITY SKINNY_64_64_parallel IS
	PORT (
		CLK : IN STD_LOGIC;
...
----Sub Components Definitions
	...
	COMPONENT AddRoundTweakey
		PORT (
			tweakey_in : IN std_logic_vector(31 DOWNTO 0);
	...
	-- Skinny key schedule operation
	COMPONENT tw_schedule
		PORT (
			TWEAKEY_SCHEDULE_IN : IN std_logic_vector(63 DOWNTO 0);
...
------------------------------------------------------------------------------------------------------------	
	...
	SIGNAL OUTPUT_MUX_OUT : std_logic_vector(63 DOWNTO 0);

	SIGNAL IS_REG_IN : std_logic_vector(63 DOWNTO 0);
	SIGNAL IS_REG_OUT : std_logic_vector(63 DOWNTO 0);
	SIGNAL TW_REG_IN : std_logic_vector(63 DOWNTO 0);
	SIGNAL TW_REG_OUT : std_logic_vector(63 DOWNTO 0);

	SIGNAL key_schedule_out : std_logic_vector(63 DOWNTO 0);
	...
	SIGNAL TW_SCHEDULE_OUT : std_logic_vector(63 DOWNTO 0);
...
BEGIN
	...
	PORT MAP(
		tweakey_in => TW_REG_OUT(63 DOWNTO 32), -- only first two rows are xored
	...
	BEGIN
		...
		CASE current_state IS
			...
			WHEN processing =>
                ...
                -- State transition 
				IF lfsr_out = b"111000" THEN    --lfsr out is used to count up rounds, when it comes to his 32th value encrypt is done.
					OUT_MUX_SEL <= '1';    --output the correct ciphertext
					BUSY <= '0';    -- Busy flag goes down when encrypt is done
