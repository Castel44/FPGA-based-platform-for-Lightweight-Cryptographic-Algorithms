...
-- Subcomponents delcaraton:
	-- DUT: Simon with Block size = 128 bit; Key size = 128 bit
	-- Datapath = 64 bit
	COMPONENT Simon_128_128_parallel IS
		PORT (
...
-- Component Instantiation
	...
	-- Cipher under Test
	Simon_DUT : Simon_128_128_parallel
	PORT MAP(
		clk => clk,
	...
	BEGIN
		CASE current_state IS
			...
			WHEN waiting =>
				...
				data_ready_W <= '0';
				start_W <= '0';
				plaintext_reg <= plaintext_tst(Datapath - 1 + Datapath * (to_integer(unsigned(cnt_out_W))) DOWNTO 0 + Datapath * (to_integer(unsigned(cnt_out_W))));
				key_reg <= key_tst(Datapath - 1 + Datapath * (to_integer(unsigned(cnt_out_W))) DOWNTO 0 + Datapath * (to_integer(unsigned(cnt_out_W))));

				-- output ports 
				led_out <= '0';

				-- transition  
                -- Since datapath is 64 bit, it takes 2 clk clycle to correct load new key and plaintext
				IF cnt_out_W = b"01" THEN
					nx_state <= start_enc;
				ELSE
					nx_state <= waiting;
				END IF;
