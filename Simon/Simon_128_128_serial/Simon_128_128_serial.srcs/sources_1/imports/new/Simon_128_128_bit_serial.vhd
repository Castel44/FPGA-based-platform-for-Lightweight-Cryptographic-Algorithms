...
BEGIN
	...
	-- element counter 
	-- this counter is used to count the individual bits processed by the cipher which are 64	
	INST_SERIAL_CNT : cnt
	GENERIC MAP(size => 6)
	PORT MAP(
	...
	-- the lfsr should change his value every round, so when all the 24 individual bits are processed	
	INST_lfsr : lfsr
	PORT MAP(
		lfsr_out => lfsr_out,
		clk => lfsr_change,
	...
	-- This 2 bit counter will be used in the state machine to determine whether the cipher has done its work.		
		...
		CASE current_state IS
			...
			WHEN processing =>
				...
				IF lfsr_parallel_out = b"11001" THEN
					...
					IF end_cnt_out = b"10" THEN		-- only if this condition is met the cipher has finished
													-- this is because we are using the lfsr also for the loading phase
													-- we used this counter trick to spare logic
						IS_CE <= '0';
					...
					IF serial_cnt_out = b"111111" THEN	-- if the round has ended 
						end_cnt_ce <= '1';				-- let the counter increase
					ELSE
						end_cnt_ce <= '0';
					END IF;
