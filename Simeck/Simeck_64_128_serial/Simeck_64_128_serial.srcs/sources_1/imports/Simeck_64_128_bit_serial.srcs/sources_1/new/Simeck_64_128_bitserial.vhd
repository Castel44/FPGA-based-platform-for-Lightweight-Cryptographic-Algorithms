...
ARCHITECTURE Behavioral OF Simeck_64_128_bit_serial IS
------------------------------------------------------------------------------------------------------------
--Sub Components Definitions
	...
	COMPONENT lfsr
		GENERIC (
			length : INTEGER; -- determine the primitive polynomial and reg length
	...
	BEGIN
		CASE current_state IS
			...
			WHEN loading =>
				...
				ELSIF lfsr_parallel_out = b"111000" THEN -- keep loading key
					IS_CE <= '0';
					KEY_REG_CE <= '1';
					nx_state <= loading;

				ELSIF lfsr_parallel_out = b"110000" THEN -- key has been loaded go to idle
					IS_CE <= '0';
					KEY_REG_CE <= '0';
					nx_state <= idle;
			...
			WHEN processing =>
				...
				--state transition
				IF lfsr_parallel_out = b"011011" THEN 
					IS_CE <= '0';
