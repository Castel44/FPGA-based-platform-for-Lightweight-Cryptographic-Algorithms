...
ENTITY Simeck_64_128_parallel IS
	GENERIC (
		Datapath : INTEGER RANGE 0 TO 32 := 32;
...
ARCHITECTURE Behavioral OF Simeck_64_128_parallel IS
	...
	COMPONENT normal_shift_reg
		GENERIC (
			width : INTEGER := 32; -- shift_reg width 
    ...
    -- Shift Register with right and left 32 bits output (length is 2)
	...
	-- left in is the leftmost portion of the internal state (first 32 bits)
	-- rigth_ in the rightmost (last 32 bits)
	...
	-- his primitive polynomial is X6 + X + 1
	COMPONENT lfsr
		PORT (
...
BEGIN
    ...
    -- Internal State mux, correctly route signals for the IS
	INST_INMUX : MUX
	GENERIC MAP(
		datapath => Datapath
	)
	...
	BEGIN
		CASE current_state IS
			...
			WHEN processing =>
                ...
                -- State transition
                -- lfsr output values is used to determine when encrypt is done				
			...
			WHEN end_encrypt =>
                ...
                -- State transition
				IF lfsr_parallel_out = b"110111" THEN	 -- count up to 2 clk cycles 
