...
------------------------------------------------------------------------------------------------------------
	...
	-- DUT: Simeck with Block size = 48 bit; Key size = 96 bit
	-- Datapath = 1 bit
	COMPONENT Simeck_48_96_serial IS
		PORT (
...
------------------------------------------------------------------------------------------------------------
	...
	SIGNAL plaintext_tst : std_logic_vector(Datapath * 4 - 1 DOWNTO 0) := (X"000000000000" & X"72696320646e");	-- plaintext with some zeros concatenate on top of it, makes easier the correct loading into Internal state register	
	SIGNAL correct_ciphertext : std_logic_vector(Datapath * 2 - 1 DOWNTO 0) := X"f3cf25e33b36";
...
-- Component Instantiation
	...
	-- Cipher under Test
	Simeck_DUT : Simeck_48_96_serial
	PORT MAP(
		clk => clk,
	...
	BEGIN
		CASE current_state IS
			...
			WHEN waiting =>
				...
				-- State transition   
				-- Since datapath is 1 bit, it takes 96 clk clycle to correct load new key and plaintext
				IF cnt_out_W = b"1011111" THEN
					nx_state <= start_enc;
				ELSE
					nx_state <= waiting;
				END IF;
			...
			WHEN ending =>
				...
				IF ciphertext_out_W = correct_ciphertext(1 + to_integer(unsigned(cnt_out_W)) DOWNTO 1 + to_integer(unsigned(cnt_out_W))) THEN
				    -- check the first 96 value of ciphertext (95 here, 1 on previous state)					
					IF cnt_out_W = b"101110" THEN
						nx_state <= success;
					ELSE
						nx_state <= ending;
					END IF;
